...
export default async function LocationSpecificOutdoorLightingPage({ params }: { params: { location: string } }) {
  ...
  const structuredDataProps = {
    ...
    price: serviceContent.pricing?.startingAt?.replace(/[^0-9]/g, '') || "2500", // Default price for structured data
    ratingValue: "4.9",
    reviewCount: "87",
    testimonial: {
  ...
  return (
    <LocationServicePageTemplate
      ...
      serviceAreaText={`We provide professional outdoor lighting services throughout ${cityName} and surrounding areas.`}
      ctaTitle={`Need Help With Your Outdoor Lighting in ${cityName}?`}
      ctaSubtitle={`Contact our team today for expert lighting solutions in ${cityName}.`}
      articleTitle={`Additional Information About Outdoor Lighting in ${cityName}`}
    />
  );
}
