// A script to ensure all service data files have entries for all locations
const fs = require('fs');
const path = require('path');

// Service types and their corresponding file names
const SERVICE_FILES = [
...
async function generateMissingLocationData() {
  ...
  for (const service of SERVICE_FILES) {
    await processServiceFile(service.slug, service.fileName, allLocations);
  }
...
async function processServiceFile(serviceSlug, fileName, allLocations) {
  ...
  if (!fs.existsSync(dataFilePath)) {
    console.log(`Service data file ${fileName} doesn't exist. Skipping.`);
    return;
  }
  ...
  // Find the end of the template block
  let endIndex = -1;
  let braceCount = 0;
  let inQuote = false;
  let escaped = false;
  ...
  if (endIndex === -1) {
    console.log(`- Could not find end of template block. Skipping.`);
    return;
  }
  
  templateBlock = fileContent.substring(startIndex, endIndex);
  ...
  const insertPoint = fileContent.indexOf('{', fileContent.indexOf('Record<string, ServiceContent>')) + 1;
  ...
  console.log(`- Successfully updated ${fileName} with ${count} new locations`);
}
